...
export const KitModel = {
  async findAll(activeOnly = false): Promise<KitOutput[]> {
  ...
  async findById(id: string): Promise<KitOutput | null> {
    const query = `
      SELECT 
        k.*,
        COALESCE(
  ...
  async create(data: KitInput): Promise<KitOutput> {
    ...
    try {
      ...
      const kitRes = await client.query(
        `INSERT INTO kits (name, slug, description, price, badge_text, banner_image, active, featured)
      ...
      if (data.items && data.items.length > 0) {
        for (const it of data.items) {
          await client.query(
            `INSERT INTO kit_items (kit_id, product_id, qty, selected_color)
             VALUES ($1, $2, $3, $4)`,
            [kit.id, it.product_id, it.qty || 1, it.selected_color || null]
          );
        }
      }
  ...
  async update(id: string, data: Partial<KitInput>): Promise<KitOutput | null> {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      if (data.badge_text !== undefined) { fields.push(`badge_text = $${idx++}`); values.push(data.badge_text); }
      ...
      if (fields.length > 1) {
        values.push(id);
        await client.query(
          `UPDATE kits SET ${fields.join(', ')} WHERE id = $${idx}`,
          values
        );
      }
      ...
      if (data.items) {
        ...
        for (const it of data.items) {
          await client.query(
            `INSERT INTO kit_items (kit_id, product_id, qty, selected_color)
             VALUES ($1, $2, $3, $4)`,
            [id, it.product_id, it.qty || 1, it.selected_color || null]
          );
        }
      }
  ...
  calculateKitMetrics(rawKit: any): KitOutput {
